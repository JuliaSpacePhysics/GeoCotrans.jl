...
- `rholim::Real = 40.0`: Maximum cylindrical radius √(x² + y²) (Earth radii)
...
```
"""
function FieldLineCallback(; r0::Real = 1.0, rlim::Real = 10.0, xlim::Real = 20.0, rholim::Real = 40.0)
    ...
    # Cylindrical radius boundary
    function rho_condition(u, t, integrator)
        rho = sqrt(u[1]^2 + u[2]^2)
        return rholim - rho
    end
    rho_cb = ContinuousCallback(rho_condition, terminate!)

    return CallbackSet(inner_cb, outer_cb, x_cb, rho_cb)
end
